;(() => {
    ...
    // Función para dar like/dislike a un comentario
    function setCommentLikeDislike(commentId, isLike) {
        return fetch('/like-dislike-comment', {
            method: 'POST',
        ...
        .then(r => r.json())
        .catch(err => {
            console.error("Error en setCommentLikeDislike:", err);
            return { success: false };
        });
    }

    // Manejar clicks en like/dislike de comentarios
    document.addEventListener('click', event => {
        ...
        if (likeBtn) {
            ...
            setCommentLikeDislike(commentId, true).then(data => {
                if (data.success) {
                    // Actualizar contadores
                    const likeCountEl = document.querySelector(`.like-comment-count[data-comment-id="${commentId}"]`);
                    const dislikeCountEl = document.querySelector(`.dislike-comment-count[data-comment-id="${commentId}"]`);
        ...
        if (dislikeBtn) {
            ...
            setCommentLikeDislike(commentId, false).then(data => {
                if (data.success) {
                    // Actualizar contadores
                    const likeCountEl = document.querySelector(`.like-comment-count[data-comment-id="${commentId}"]`);
                    const dislikeCountEl = document.querySelector(`.dislike-comment-count[data-comment-id="${commentId}"]`);
